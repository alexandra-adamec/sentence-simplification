'''
Bi-directional RNN Encoder-Decoder model with Long Short-Term memory
'''
...
logging.basicConfig(format=FORMAT, level=logging.DEBUG, filename='/tmp/log/log2.txt')


class RecEncoder(object):
    '''
    Recurrent encoder, that takes as input a variable-length sequence of indices
    and returns a fixed-length vector of its encoding.
    '''
    def __init__(self, init_emb, hidden_size, backwards=False):
        ...
        def encode(x_j, h_jm1, mem_jm1):
            # LSTM computation of the hidden state at j
        ...
        [self.encoding, _], _ = theano.scan(fn=encode, sequences=[input_emb], outputs_info=[T.alloc(h0, input_emb.shape[1], hidden_size), T.alloc(mem0, input_emb.shape[1], hidden_size)], n_steps=input_emb.shape[0], go_backwards=backwards)
...
class RecDecoder(object):
    ...
    def __init__(self, encoder, context, voc_size, init_emb, context_size, hidden_size, al_size, beam_size, start_symbol_idx = 0.0):
        ...
        wa = theano.shared(name='wa', value=0.001 * np.random.uniform(-1.0, 1.0, (hidden_size, al_size)).astype(theano.config.floatX))
        ua = theano.shared(name='ua', value=0.001 * np.random.uniform(-1.0, 1.0, (2*context_size, al_size)).astype(theano.config.floatX))
        va = theano.shared(name='va', value=0.001 * np.random.uniform(-1.0, 1.0, al_size).astype(theano.config.floatX))
        ...
        fi = theano.shared(name='fi', value=0.08 * np.random.uniform(-1.0, 1.0, (2*context_size, hidden_size)).astype(theano.config.floatX))
        ff = theano.shared(name='ff', value=0.08 * np.random.uniform(-1.0, 1.0, (2*context_size, hidden_size)).astype(theano.config.floatX))
        fo = theano.shared(name='fo', value=0.08 * np.random.uniform(-1.0, 1.0, (2*context_size, hidden_size)).astype(theano.config.floatX))
        fc = theano.shared(name='fc', value=0.08 * np.random.uniform(-1.0, 1.0, (2*context_size, hidden_size)).astype(theano.config.floatX))
        ...
        g = theano.shared(name='g', value=0.08 * np.random.uniform(-1.0, 1.0, (2*context_size, voc_size)).astype(theano.config.floatX))
        by = theano.shared(name='by', value=np.zeros(voc_size, dtype=theano.config.floatX))

        # deltas of the parameters store previous parameters updates,
        # used to compute the momentum in the next iteration of the training
        delta_wa = theano.shared(value=np.zeros(wa.get_value().shape, dtype=theano.config.floatX))
        delta_ua = theano.shared(value=np.zeros(ua.get_value().shape, dtype=theano.config.floatX))
        delta_va = theano.shared(value=np.zeros(va.get_value().shape, dtype=theano.config.floatX))
        ...
        self.params = [wa, ua, va, ui, uf, uo, uc, wi, wf, wo, wc, fi, ff, fo, fc, d, v, g, bias_s, by]
        self.deltas = [delta_wa, delta_ua, delta_va, delta_ui, delta_uf, delta_uo, delta_uc, delta_wi, delta_wf, delta_wo, delta_wc, delta_fi, delta_ff, delta_fo, delta_fc, delta_d, delta_v, delta_g, delta_bias_s, delta_by]
        ...
        def sent_dot(a_vec, h_mat):
            return T.dot(a_vec, h_mat)


        def decode(y_im1, s_im1, mem_im1, hh):
            # alignment model
            a_i = T.nnet.softmax(T.dot(T.tanh(T.dot(s_im1, wa)[:, np.newaxis, :] + T.dot(hh, ua)), va))
            c_i, _ = theano.scan(fn=sent_dot, sequences=[a_i, hh], outputs_info=None)

            # LSTM computation
            i_i = T.nnet.sigmoid(T.dot(emb[y_im1], ui) + T.dot(s_im1, wi) + T.dot(c_i, fi) + bias_s)
            f_i = T.nnet.sigmoid(T.dot(emb[y_im1], uf) + T.dot(s_im1, wf) + T.dot(c_i, ff) + bias_s)
            o_i = T.nnet.sigmoid(T.dot(emb[y_im1], uo) + T.dot(s_im1, wo) + T.dot(c_i, fo) + bias_s)
            mem_i = T.tanh(T.dot(emb[y_im1], uc) + T.dot(s_im1, wc) + T.dot(c_i, fc) + bias_s)
            ...
            y_i = T.nnet.softmax(T.dot(emb[y_im1], d) + T.dot(s_i, v) + T.dot(c_i, g) + by)
        ...
        def predict_word(y_im1, s_im1, mem_im1, loglkl_im1, hh):
            # alignment model
            a_i = T.nnet.softmax(T.dot(T.tanh(T.dot(s_im1, wa)[:, np.newaxis, :] + T.dot(hh, ua)), va))
            c_i, _ = theano.scan(fn=sent_dot, sequences=[a_i, hh], outputs_info=None)

            # LSTM
            i_i = T.nnet.sigmoid(T.dot(emb[y_im1], ui) + T.dot(s_im1, wi) + T.dot(c_i, fi))
            f_i = T.nnet.sigmoid(T.dot(emb[y_im1], uf) + T.dot(s_im1, wf) + T.dot(c_i, ff))
            o_i = T.nnet.sigmoid(T.dot(emb[y_im1], uo) + T.dot(s_im1, wo) + T.dot(c_i, fo))
            mem_i = T.tanh(T.dot(emb[y_im1], uc) + T.dot(s_im1, wc) + T.dot(c_i, fc))
            fmem_i = (f_i * mem_im1) + (i_i * mem_i)
            s_i = o_i * T.tanh(fmem_i)
        ...
        def process_hypothesis(y_im1, s_im1, mem_im1, loglkl_im1, hh):
            y_i, s_i, mem_i, loglkl = predict_word(y_im1, s_im1, mem_im1, loglkl_im1, hh)
        ...
        def generate(y_im1, s_im1, mem_im1, log_lkl_im1, all_pred_y_im1, iter, hh):
            # process all hypothesis, for each get a vocabulary distribution
            [_, tmp_s_i, tmp_mem_i, tmp_loglkl],_ = theano.scan(fn=process_hypothesis, sequences=[y_im1.dimshuffle((1,0)), s_im1.dimshuffle((1,0,2)), mem_im1.dimshuffle((1,0,2)), log_lkl_im1.dimshuffle((1,0))], non_sequences=hh)
...
class SimplRNN(object):
    '''
    RNN model joinning forward and backward encoders and decoder.
    '''
    def __init__(self, voc_main, voc_simple, voc_main_emb, voc_simple_emb, h_size, s_size, al_size, enc_l2, dec_l2, lr_init, lr_decay, mom, batch_size, beam_size, params_file=None):
        ...
        encoder_fwd = RecEncoder(voc_main_emb, h_size)
        encoder_bkw = RecEncoder(voc_main_emb, h_size, backwards=True)
        self.encoding = T.concatenate([encoder_fwd.encoding.dimshuffle((1, 0, 2)), encoder_bkw.encoding.dimshuffle((1, 0, 2))], axis=2)

        decoder = RecDecoder(encoder_fwd, self.encoding, voc_simple.size(), voc_simple_emb, h_size, s_size, al_size, beam_size)
        self.params = encoder_fwd.params + encoder_bkw.params + decoder.params
        self.deltas = encoder_fwd.deltas + encoder_bkw.deltas + decoder.deltas
        ...
        enc_l2_reg = enc_l2 * T.sum([T.sum(p ** 2) for p in encoder_fwd.params[:-1] + encoder_bkw.params[:-1]])
        ...
        self.train_fn = theano.function(
            ...
            givens={
                encoder_fwd.input_batch: self.train_main[idx_from:idx_to],
                encoder_bkw.input_batch: self.train_main[idx_from:idx_to],
        ...
        self.encode_test_fn = theano.function(
            ...
            givens={
                encoder_fwd.input_batch: self.test_main,
                encoder_bkw.input_batch: self.test_main
            }
        )
    ...
    def train(self, data_set, n_epochs):
        ...
        for n in range(n_epochs):
            ...
            path = '/tmp/param/tmp_%i_brnn_lstm_%s_%s.pickle' % (n, datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S'), self.model_desc)
        ...
        path = '/tmp/param/brnn_lstm_%s_%s.pickle' % (datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S'), self.model_desc)
    ...
    def test(self, data_set):
        ...
        path = '/tmp/output/%s_model_%s.txt' % (datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S'), self.model_desc)
        path2 = '/tmp/output/raw_%s_model_%s.txt' % (datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S'), self.model_desc)
